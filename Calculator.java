...
if(delims.indexOf(temp)<0){neo.push(temp);} //if the token is a value, push it onto the stack

else if(temp.equals(" ")){continue;} // ignore spaces ?
...
String num2 = neo.pop();
neo.push(math(num1,temp,num2));//evaluate the operater with values as arguments and push the results back to stack 
}

}

if(neo.size()>1){System.out.println("Not a valid expression!");
return 0.0;}
else{
String ans = neo.pop();
...
if(delims.indexOf(temp)<0){output+= temp +" ";}//if token is a number then add to output
...
public static void compareToPrecedence(AStack opstack, String qq, AStack numstack){ //qq is temp, operator being read in
if(opstack.empty()){
opstack.push(qq);
return;}
...
if(qq.equals("(")){out_precedence = 0;}
else  out_precedence= getOrder(qq);
...
else if(qq.equals(")")&& in.equals("(")){return;}
...
try{thing = "" + num2/num1;
return thing;}
catch(ArithmeticException e){System.out.println("Error: Divided by zero");}
return thing;
}

else if(operator.equals("^")){return ""+(Math.pow(num2,num1));}
else{
...
if(thing.equals(")")){return 1;}
else if(thing.equals("^")){return 2;}
else if(thing.equals("/") || thing.equals("*")){return 3;}
else if(thing.equals("+") || thing.equals("-")){return 4;}
else if(thing.equals("(")){return 5;}
else return-1;}
...
if(qwerty.equals("{")){a++;}
else if(qwerty.equals("}")){b++;}
else if(qwerty.equals("(")){d++;}
else if(qwerty.equals(")")){e++;}
else if(qwerty.equals("[")){f++;}
else if(qwerty.equals("]")){g++;}
else if(qwerty.equals("<")){h++;}
else i++;
}

if(a==b && d==e && f==g && h==i){return true;}
else 
return false;
}
